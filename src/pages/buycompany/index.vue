<template>
  <div class="container">
    <div class="head">
      <van-steps :steps="steps" :active="active"  active-color="#5596fa"/>
    </div>
    <div class="first" v-if="active==0">
      <van-cell-group>
        ...
        <van-field
          ...
          placeholder="请输入联系人"
          input-align="right"
           @input="contacts=$event.mp.detail"
          />
        ...
        <van-field
          ...
          placeholder="请输入详细地址"
          input-align="right"
           @input="addressDetails=$event.mp.detail"
        />
        <van-field
          :value=" registCode "
          label="工商注册号"
          ...
          input-align="right"
           @input="registCode=$event.mp.detail"
        />
        <van-field
          label="申请时间"
          :value='timer'
 ...
 -->
    ...
    <div class="second" v-else-if="active==1">
      <van-cell-group>
        ...
        <van-popup
          ...
          @close="onClose2">
          <div class="pick">
            <van-picker title="平台结构类型" show-toolbar :columns="columns2" value-key="dictLabel" @cancel="show2 = false" @confirm="onConfirm2"/>
          </div>
        </van-popup>
        ...
        <van-popup
          ...
          @close="onClose3">
          <div class="pick">
            <van-picker title="请选择用途" show-toolbar :columns="columns3" value-key="dictLabel"   @cancel="show3 = false" @confirm="onConfirm3"/>
          </div>
        </van-popup>
        <van-field
          :value=" scale1 "
        ...
        <van-popup
          ...
          @close="onClose4">
          <div class="pick">
            <van-picker title="请选择飞行器等级" show-toolbar :columns="columns4" value-key="dictLabel" @cancel="show4 = false" @confirm="onConfirm4"/>
          </div>
        </van-popup>
    ...
    <div class="foot">
      <div class="finish" @click="next">下一步</div>
    </div>
...
export default {
  ...
  data () {
    return {
     ...
     columns3:[],
     columns4:[],
     columns5:['超近程无人机','近程无人机','短程无人机','中程无人机','远程无人机',],
     columns6:[],
     columns7:[],
     columns8:[],
     columns10:["是","否"],
     steps: [{desc: '单位信息',},{desc: '飞行器信息',},{desc: '完成',}],
     areaList:{
  ...
  methods:{
    ...
    onConfirm6(e) {
     this.registTime =e.mp.detail
     var oDate = new Date(this.registTime)
    ...
    afterQy(event) {
      ...
      mpvue.uploadFile({
        ...
        success(res) {
          ...
          that.qyimg= process.env.VUE_APP_IMAGES+that.fileName1
        },
      });
    },
    //驾照
    afterzz(event) {
      ...
      mpvue.uploadFile({
        ...
        success(res) {
          ...
          that.zzimg= process.env.VUE_APP_IMAGES+that.fileName2
        },
      });
    },
    //查询结构类型
   ...
   //分步骤跳转
    next(){
      let that =this
      if(this.active==0){
        if(that.name!=''&&that.phone!=''&&that.contacts!=''&&that.addressDetails!=''&&that.county!=''&&that.registCode!=''&&that.file1!=''&&that.timer!=''
        // &&/^1[3456789]\d{9}$/.test(this.phone)
          ...
          this.aaa = {
              ...
              "phone":that.phone,
              "registCode":that.registCode,
              "licenseNumber":that.licenseNumber,
              "registTime":that.registTime,
              "enclosure":that.fileName1
              }
          this.ccc={
            "licenseNumber":that.licenseNumber,
        ...
        //飞行器信息注册
        if(that.ytCnCode!=''&&that.snCode!=''&&that.manufacture!=''&&that.model!=''&&that.wheelBase!=''&&that.purpose!=''&&that.scale!=''&&that.deadWeight!=''&&that.maximumPayload!=''&&that.endurance!=''&&that.windLoading!=''&&that.enduranceMileage!=''&&that.flightSpeed!=''&&that.remoteControl!=''&&that.flightAltitude!=''&&that.propellerSize!=''
        ){
          that.bbb={
            "hsCode":that.hsCode,
          ...
          this.$http.post({
            url:'purchaseRecordController/addRecordTbEntity',
            data:{
              "tbEntity":that.aaa,
...
<style scoped>
  ...
  .zm_crad img,.fm_crad img,.port_img img,.fxzz_img img{
    width: 320rpx;
    height: 210rpx;
    position: absolute;
  }
